...
class Formulario_newton(forms.Form):
    ...
    xi = forms.FloatField(label='Xi', required=True, widget=forms.NumberInput(attrs={'placeholder':'Ingrese el x inicial...','step':'0.00000000001'}))
    iteraciones = forms.IntegerField(label='iteacion', required=True, widget=forms.NumberInput(attrs={'placeholder':'Ingrese el numero de iteraciones...'}))
    tolerancia = forms.FloatField(label='Tolerancia', required=True, widget=forms.NumberInput(attrs={'placeholder':'Ingrese la tolerancia...','step':'0.00000000001'}))
    opcion = forms.ChoiceField(label='opcion', required=True, widget=forms.RadioSelect, choices=opciones)
...
class Formulario_secante(forms.Form):
    opciones = [('1','Error absoluto'),('2','Error Relativo')]
    funcion = forms.CharField(label='Funcion fx', required=True, widget=forms.TextInput(attrs={'placeholder':'Ingrese la funcion fx...'}))
    x1 = forms.FloatField(label='X1', required=True, widget=forms.NumberInput(attrs={'placeholder':'Ingrese el x1...','step':'0.00000000001'}))
    x2 = forms.FloatField(label='X2', required=True, widget=forms.NumberInput(attrs={'placeholder':'Ingrese el x2...','step':'0.00000000001'}))
    iteraciones = forms.IntegerField(label='iteacion', required=True, widget=forms.NumberInput(attrs={'placeholder':'Ingrese el numero de iteraciones...'}))
    tolerancia = forms.FloatField(label='Xi', required=True, widget=forms.NumberInput(attrs={'placeholder':'Ingrese la tolerancia...','step':'0.00000000001'}))
    opcion = forms.ChoiceField(label='opcion', required=True, widget=forms.RadioSelect, choices=opciones)